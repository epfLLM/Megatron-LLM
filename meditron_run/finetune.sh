...
LOAD_CHECKPOINT_PATH=/pure-mlo-scratch/alhernan/megatron-data/checkpoints-backup/${MODEL}-${SIZE}b-tp$TP-pp$PP-optim
...
if [[ $ITER > 0 ]]; then
	bash /pure-mlo-scratch/trial-runs/${EXP_NAME}/checkpoints/tmp/tag.sh $ITER $EXP_NAME
...
TRAIN_DATA_PATH=/pure-mlo-scratch/data/tokenized/pubmed-replay/pubmed-train-replay-400M-llama_text_document
VALID_DATA_PATH=/pure-mlo-scratch/data/tokenized/pubmed-all-validation/pubmed-all-validation-llama_text_document
...
COMMON_ARGS="--use_flash_attn --no_bias_gelu_fusion
		...
		--log_validation_ppl_to_tensorboard --log_memory_to_tensorboard
...
if [[ $WANDB = 1 ]]; then
	COMMON_ARGS="$COMMON_ARGS --wandb_logger --wandb_project meditron7b --wandb_name $EXP_NAME-test-$TEST-$ITER"
fi
...
CUDA_DEVICE_MAX_CONNECTIONS=1 OMP_NUM_THREADS=16 torchrun $DISTRIBUTED_ARGS /main/finetune.py \
